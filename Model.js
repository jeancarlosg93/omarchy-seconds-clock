...
function clockFormats(vertical) {
  var presets = vertical ? VERTICAL_CLOCK_FORMATS : CLOCK_FORMATS
  return presets.map(function(format) { return secondsFormat(format, vertical) })
}

// Enabling a clone preserves the original clock's format in shell.json.
// Keep seconds visible even when that saved format only specified minutes.
function secondsFormat(format, vertical) {
  var value = String(format === undefined || format === null ? "" : format)
  if (value.indexOf("ss") !== -1) return value
  if (value.indexOf("mm") !== -1)
    return value.replace("mm", vertical ? "mm\nss" : "mm:ss")
